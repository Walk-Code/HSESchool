...
public class StoreCheckOrderActivity extends BaseActivity{
	...
	protected void onCreate(Bundle arg0) {
		...
		if(mShoppingCarArr == null || 0 == mShoppingCarArr.length()){
			return;
		}
		initView();
			...
			public void handleMessage(Message msg) {
				...
				case EventManager.EventType_UpdateAddress:
					if(objs == null || objs.length == 0){
						...
						mCurrentAddressId = (String) objs[4];
					}
					break;
				...
				case EventManager.EventType_UpdateAddressPicker:
					if(mCurrentAddressId != null && mCurrentAddressId.equalsIgnoreCase((String)objs[0])){
						((TextView)mListViewHeaderView.findViewById(R.id.store_consignee)).setText((String)objs[1]);
						((TextView)mListViewHeaderView.findViewById(R.id.store_consignee_phone)).setText((String)objs[2]);
						((TextView)mListViewHeaderView.findViewById(R.id.store_address)).setText((String)objs[3]);
					}
					break;
				default:
					break;
	...
	private void initView() {
		...
		for(int i = 0; i < mShoppingCarArr.length(); i++){
			...
			if(obj != null){
				String deliverType = obj.optString("deliver");
    			if(!Utils.isTextEmpty(deliverType) && deliverType.equalsIgnoreCase("1")){
    				mMeansMap.put(obj.optString("shopId"), 1);
    			}
    			else{
		...
		mListView.setAdapter(mListAdapter = new ContentAdapter(){
			...
			public View getView(int position, View convertView, ViewGroup parent) {
    			...
    			((TextView)convertView.findViewById(R.id.textview_deliver_address)).setText(getResources().getString(R.string.goods_consignee_shopname) + obj.optString("address"));
    			...
    			if(!Utils.isTextEmpty(deliverType) && !deliverType.contains(",")){
    				convertView.findViewById(R.id.store_send_goods).setEnabled(false);
    				convertView.findViewById(R.id.arrow).setVisibility(View.GONE);
    			}
    			else {
    				convertView.findViewById(R.id.store_send_goods).setEnabled(true);
				...
				case 0:
					convertView.findViewById(R.id.order_freight).setVisibility(View.VISIBLE);
					convertView.findViewById(R.id.group_deliver_address).setVisibility(View.GONE);
					...
					((TextView)convertView.findViewById(R.id.store_shopping_delivery)).setText(getResources().getString(R.string.store_shopping_delivery));
					if(!obj.has("deliverFee") || Utils.isTextEmpty(obj.optString("deliverFee")) || obj.optDouble("deliverFee", 0) == 0){
						deliverFee = 0.00;
						convertView.findViewById(R.id.order_freight).setVisibility(View.GONE);
					}
					else{
				...
				case 1:
					convertView.findViewById(R.id.order_freight).setVisibility(View.GONE);
					convertView.findViewById(R.id.group_deliver_address).setVisibility(View.VISIBLE);
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if(arg2 == 0){
					...
					Intent intent = new Intent(StoreCheckOrderActivity.this, StoreAddressPickerActivity.class);
	    ...
	    findViewById(R.id.group_left).setOnClickListener(new  OnClickListener() {
	...
	public String comBination(JSONArray json,Deliver deliver){
		...
		for(int i = 0 ; i < json.length(); i++){
			...
			try {
				double money = 0;//jsonObject.optDouble("money");
				String totalPrice = ((TextView)findViewById(R.id.total_price)).getText().toString();
				if(!Utils.isTextEmpty(totalPrice) && totalPrice.length() > 0){
					money = Double.parseDouble(totalPrice.substring(1, totalPrice.length()));
				}
	...
	private void popupWindow(String address, final String shopId){ 
		 ...
		 view.findViewById(R.id.store_shopping_delivery_layout).setOnClickListener(new OnClickListener() {	
			...
			@Override
			public void onClick(View v) {					
				((ImageView)view.findViewById(R.id.store_shopping_delivery)).setImageResource(R.drawable.list_red_option_tick);
		 ...
		 view.findViewById(R.id.store_buyer_way_layout).setOnClickListener(new OnClickListener() {			
			@Override
			public void onClick(View v) {	
				((ImageView)view.findViewById(R.id.store_buyer_way)).setImageResource(R.drawable.list_red_option_tick);
		...
		((TextView)view.findViewById(R.id.store_goods_place)).setText(getResources().getString(R.string.store_checkorder_goods_place) + address);
	...
	@Override
	public void taskFinished(TaskType type, final Object result, boolean isHistory) {
			...
			if(((JSONObject)result).has("items")){
				...
				if(itmeArr != null && itmeArr.length() > 0){
					...
					if(showObj != null){
						...
						mCurrentAddressId = showObj.optString("id");
					}
				}
				else{
		...
		case TaskOrMethod_GetWay:
			...
			new Handler().postDelayed(new Runnable() {
				...
				public void run() {
					...
					if(mPayType.equalsIgnoreCase("2")){
