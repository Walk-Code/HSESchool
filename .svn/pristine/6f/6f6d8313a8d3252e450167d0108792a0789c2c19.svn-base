...
public class SchoolifeFragment extends BaseFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mMainLayout = (ViewGroup) View.inflate(mActivity, R.layout.fragment_schoollife, null);
		initListView();		
		...
		EventManager.getInstance().setHandlerListenner(mHandler = new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				case EventManager.EventType_ShoppingCar:				
					if((int)obj[0] != 0){
						mMainLayout.findViewById(R.id.store_car_count).setVisibility(View.VISIBLE);
						((TextView)mMainLayout.findViewById(R.id.store_car_count)).setText((int)obj[0] == 0 ? "" :(int)obj[0]+"");
					}else{
						(mMainLayout.findViewById(R.id.store_car_count)).setVisibility(View.GONE);						
					}					 
				
					break;	
				case EventManager.EventType_StoreHomepageLoad:
					if(mListView != null){
	...
	public void initListView(){		
		...
		mListView.setAdapter(new ContentAdapter(){
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				if(convertView == null){
					convertView = View.inflate(mActivity, R.layout.listcell_myschoolyard, null);
	...
	private void initHeaderView() {	
		...
		mBannerGallery.setAdapter(new ContentAdapter(){
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				if(convertView == null){
					convertView = View.inflate(mActivity, R.layout.listcell_myschoolyard_headercell, null);				
	  ...
	  mHeaderView.findViewById(R.id.commdoity_goods_layout).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				intent.putExtra("Data", mGoodsType);
				intent.putExtra("category", items);
				intent.putExtra("count", mShoppingCarCount);
				startActivity(intent);
			}
	  });
		...
		case TaskOrMethod_MallHomeHome:
			...
			if(result instanceof JSONObject ){		
				...
				if(((JSONObject)result).has("goodsList")){
					...
					LinearLayout.LayoutParams lp = (android.widget.LinearLayout.LayoutParams) mGoodsGridView.getLayoutParams();
					if(mGoodsListArr != null && mGoodsListArr.length() > 0){
						int line = 0;
						int count = mGoodsListArr.length() % 2;
						if(count != 0){
							line = mGoodsListArr.length() / 2 + 1;
						}
						else{
							line = mGoodsListArr.length() / 2;
						}
						lp.height = Utils.dipPx(mActivity, 190 * line);
					}
					else{
						lp.height = 0;
					}
					mGoodsGridView.setLayoutParams(lp);
					if(null != mGoodsListArr && mGoodsListViewAdapter != null){
				...
				if(DataLoader.getInstance().isLogin()){
					if(((JSONObject)result).optInt("count") != 0){
						...
						//EventManager.getInstance().sendMessage(EventManager.getInstance().EventType_ShoppingCar, mShoppingCarCount);
					}else{
						(mMainLayout.findViewById(R.id.store_car_count)).setVisibility(View.GONE);						
					}					 
				}else{
